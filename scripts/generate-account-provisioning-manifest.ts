...
function parseArgs(): Args {
  ...
  if (!input) {
    throw new Error('Usage: bun run scripts/generate-account-provisioning-manifest.ts <xlsx> [--out path] [--csv path] [--domain xigu.team] [--batch name] [--external-type employee]');
  }
  ...
  return {
    ...
    domain: option('domain', 'xigu.team').replace(/^@/, '').toLowerCase(),
    batch,
...
async function main() {
  ...
  const records: ProvisioningRecord[] = normalized.map(row => ({
