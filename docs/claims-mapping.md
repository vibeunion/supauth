...
OAuth `scope` is protocol metadata, not an enterprise permission claim. Preserve the granted standard scope string (`openid`, `email`, `profile`, `phone`, `offline_access`) in OAuth access tokens so UserInfo, ID-token, and refresh-token behavior stays Supabase-compatible. GoTrue v2.195+ advertises `offline_access` for clients that require durable refresh sessions; SupaOAuth forwards it without introducing a runtime scope allowlist. The token endpoint response may omit `scope` when it is unchanged from the request, so compatibility checks validate the JWT claim and accept an optional matching response field. Keep database authorization in RLS using `auth.jwt() ->> 'client_id'`, `auth.uid()`, and SupaOAuth permission-version lookups.
