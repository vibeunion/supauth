...
The npm packages do not move business memberships, assignments, roles, or audit ownership into SupAuth or SupaCloud. SupAuth remains the identity boundary; the application resolves authorization from its own current data after validating issuer, subject, principal kind, application, and domain.
