...
[2026-06-17 12:34:57 CST] [supaoauth] [DONE] fix(account-center-redirect-safety): 根据最终 verifier 子代理 `gpt-5.3-codex` 只读复核发现的阻断点，修复 hosted login `redirect_to` open redirect：`safeRedirectUrl(value, allowExternal=false)` 现在默认要求 `url.origin === window.location.origin` 并只返回同源 path/search/hash；只有后端 `approveAuthorization()` 返回的 OAuth `approvedRedirectUrl` 使用 `allowExternal=true`，保留已由服务端校验的应用回调能力，URL query 中自由传入的 `redirect_to=https://evil.example/...` 不再会被前端放行。同步 `packages/admin-console/static/authorize.html`、`packages/admin-console/build/authorize.html`、`packages/auth-server/src/generated/hosted-pages.ts` 与 hosted page 回归测试。重新验证：`bun test packages/auth-server/src/__tests__/account-self-service.test.ts packages/auth-server/src/__tests__/hosted-authorize-page.test.ts` 29 pass；`bun run build` 通过并导出 OpenAPI 164 paths；`bun run --filter '@supauth/auth-server' typecheck` 通过；`bun run --filter '@supauth/auth-server' test` 224 pass；`bun test` 292 pass / 42 skip / 0 fail；`bun run scripts/verify-supacloud-app-artifact.ts` ok=true；`git diff --check` 通过；敏感路径扫描未命中禁止字符串。已重新部署到 `https://auth.ai.example.team`，Function direct probe 200；installed verifier `ok=true`，manifest hash `4e6d3e8181c86515c1878cac6c9a8d19f1f357e1bd51846266a1820ac29ee7ec`；公网 smoke：`/api/v1/health` 200、`/v1/public/account/config` 200、`/v1/public/account/me` 未带 token 401、`/account` 200、`/login.html` 200 且包含同源 redirect gate、approved external only、account fallback 与 token storage。

[2026-06-17 12:30:35 CST] [supaoauth] [DONE] feat(account-center-productized): 完整补齐 Logto-like Account Center 产品化闭环。登录页在无 OAuth redirect 时把 GoTrue access token 写入 `sessionStorage` 并跳转 `/account#access_token=...`，用户不再手动粘贴 token；`/account` hosted 页面读取 hash/sessionStorage token 并调用同源 `/v1/public/account/*`。新增 MFA reset、Passkey list/rename/revoke、邮箱/手机号变更、账号注销、sessions/grants/identities/profile 模块自助 UI；新增 `/v1/public/account/config` 并让页面 `data-section`/`data-module` 与 profile fields 按 `account_center/default` 配置显隐。后端所有自助接口只接受用户 Bearer access token，先调 GoTrue `/auth/v1/user` 解析当前 userId，不接受 `x-user-id` / `x-supaoauth-user-id`，不使用旧 `/v1/my-account`，浏览器不暴露 service/admin/master token；根据 verifier M1 复核意见，后端 `requireFeature()` 现在也按 `account_center` 配置强制拒绝被关闭模块，并补充 403 回归测试；同时修复 hosted login `redirect_to` 仅允许同源路径，阻断 open redirect。已调用 executor 子代理 `gpt-5.3-codex` 完成后端自助 API 实施，verifier 子代理 `gpt-5.3-codex` 发现 M1 配置强制缺口后由主线程修复并重新验证，最终 verifier 只读复核另行派发。部署：通过 vm1 SupaCloud 管理 API 隧道 `127.0.0.1:19090` 安装最新 artifact 到 `https://auth.ai.example.team` / runtime `https://dglewlzugrtygzysqrce.ai.example.team`，Function direct probe 200；此前曾发现 Function DB secret 被通用 `.env DATABASE_URL` 覆盖为错误库，本次部署已改用项目 meta 中 `role_dglewlzugrtygzysqrce@10.20.0.1:5432/supa_dglewlzugrtygzysqrce` 的租户库连接串，未输出密钥。验证：`bun test packages/auth-server/src/__tests__/account-self-service.test.ts packages/auth-server/src/__tests__/hosted-authorize-page.test.ts` 29 pass；`bun run --filter '@supauth/auth-server' typecheck` 通过；`bun run build` 通过并导出 OpenAPI 164 paths；`bun run --filter '@supauth/auth-server' test` 224 pass；`bun test` 292 pass / 42 skip / 0 fail；`bun run scripts/verify-supacloud-app-artifact.ts` ok=true；`git diff --check` 通过；敏感路径扫描未命中禁止字符串；`SUPAUTH_INSTALLED_BASE_URL=https://auth.ai.example.team SUPAUTH_INSTALLED_RUNTIME_URL=https://dglewlzugrtygzysqrce.ai.example.team bun run scripts/verify-supacloud-installed-app.ts` ok=true；公网 smoke：`/api/v1/health` 200、`/v1/public/account/config` 200、`/v1/public/account/me` 未带 token 返回 401、`/account` 200 且包含 account config/email/phone/passkeys/delete-account 模块代码、`/login.html` 包含 `safeRedirectUrl`、`supaoauth.account.access_token`、`/account#access_token`。

[2026-06-17 11:29:15 CST] [supaoauth] [DONE] feat(account-center-modules): 继续补齐 Logto-like Account Center 的 sessions / grants / identities 模块。`/v1/public/account` 新增 `GET /sessions`、`POST /sessions/:sessionId/revoke`、`GET /grants`、`DELETE /grants/:consentId`、`GET /identities`、`DELETE /identities/:identityId`；所有接口先用用户 Bearer access token 调 GoTrue `/auth/v1/user` 解析当前用户 `id`，不接受 `x-supaoauth-user-id` / `x-user-id`，浏览器仍拿不到 service-role/admin token。sessions 与 identities 的服务端管理动作基于当前 userId 调 SupaCloud adapter；grants revoke 先列当前用户 active consents 并确认 consentId 属于该用户，避免任意撤销他人授权。`/account` hosted 页面同步打开登录会话、应用授权、登录身份三个模块：加载账号后拉取 `/v1/public/account/sessions|grants|identities`，并支持撤销会话、撤销授权、解绑身份；仍复用同源 `/v1/public` 和用户 Bearer token。同步 embedded hosted pages、OpenAPI 和回归测试。验证：`bun test packages/auth-server/src/__tests__/account-self-service.test.ts packages/auth-server/src/__tests__/hosted-authorize-page.test.ts` 22 pass；`bun run --filter '@supauth/auth-server' typecheck` 通过；`bun run --filter '@supauth/admin-console' build` 通过；`bun run build` 通过并导出 OpenAPI 154 paths；`bun run typecheck` 通过；`bun run --filter '@supauth/auth-server' test` 217 pass；`bun run scripts/verify-supacloud-app-artifact.ts` ok=true；`git diff --check` 通过。Delegation Gate: executor 子代理按 `gpt-5.3-codex` 启动，但超过 60 秒无输出，为避免并发写入冲突由主线程中断并接管；主线程完成实现、敏感路径扫描和审核。

[2026-06-17 11:21:00 CST] [supaoauth] [DONE] feat(account-center-jwt-api): 继续补齐 Logto-like Account Center 的安全 Account API 闭环。新增 `/v1/public/account/me` 与 `PATCH /v1/public/account/profile`，只接受用户 Bearer access token，服务端透传到 GoTrue `/auth/v1/user` 读取或更新当前用户资料；不接受 `x-supaoauth-user-id` / `x-user-id`，不读取浏览器传入的用户 ID，不使用 service-role/admin token。资料更新只写 GoTrue user metadata `data`，过滤 `email`、`role`、`app_metadata`、`password`、`id` 等敏感字段，成功后写 `my_account.profile.updated` audit。`/account` hosted 页面升级为可输入/读取当前用户 access token、调用同源 `/v1/public/account/me` 展示邮箱/user id/显示名称，并通过 `/v1/public/account/profile` 更新显示名称；令牌存储在 sessionStorage，支持 URL hash access_token 初始化和清除本地令牌；会话、授权、身份仍保持占位，未接入不安全旧 API。同步 embedded hosted pages、OpenAPI 和回归测试。验证：`bun test packages/auth-server/src/__tests__/account-self-service.test.ts packages/auth-server/src/__tests__/hosted-authorize-page.test.ts` 16 pass；`bun run --filter '@supauth/auth-server' typecheck` 通过；`bun run --filter '@supauth/admin-console' build` 通过；`bun run build` 通过并导出 OpenAPI 148 paths；`bun run typecheck` 通过；`bun run scripts/verify-supacloud-app-artifact.ts` ok=true；`bun run --filter '@supauth/auth-server' test` 211 pass；`git diff --check` 通过。Delegation Gate: executor 子代理按 `gpt-5.3-codex` 启动，但超过 90 秒无输出，为避免并发写入冲突由主线程中断并接管；主线程完成实施、敏感路径扫描和审核。

[2026-06-17 11:10:47 CST] [supaoauth] [DONE] feat(account-center-hosted): 补齐 minimal Logto-like Account Center 闭环。新增通用 hosted 账户中心 `/account` 与 `/account.html`，复用 sign-in experience branding 和同源 `/v1/public` 注入；当前只开放安全设置入口链接到已实现的 `/account/password`，个人资料、会话、授权和身份管理先展示为待开放，不调用现有 header-trust `/v1/my-account`，也不向浏览器暴露 service-role/admin token。新增 Admin Console `/account-center` 配置页，保存 `tenant-config account_center/default`，支持 enabled、profile edit/fields、password change、sessions、grants、identities、delete account URL；同时把 `account_center` 加入 tenant-config 白名单和通用编辑器类型。同步 hosted page generation、auth-server routes、SupaCloud manifest/installer gateway routes、artifact verifier、installed verifier 和相关测试。验证：`bun test packages/auth-server/src/__tests__/hosted-authorize-page.test.ts` 11 pass；`bun test tests/supacloud-app-contract.test.ts tests/supacloud-app-artifact-verifier.test.ts tests/supacloud-app-installer.test.ts tests/supacloud-installed-app-verifier.test.ts` 20 pass；`bun run --filter '@supauth/auth-server' typecheck` 通过；`bun run --filter '@supauth/admin-console' build` 通过；`bun run build` 通过并导出 OpenAPI 146 paths；`bun run typecheck` 通过；`bun run scripts/verify-supacloud-app-artifact.ts` ok=true；`git diff --check` 通过；build 产物包含 `packages/admin-console/build/account.html`。Delegation Gate: executor 与 verifier 子代理均按 `gpt-5.3-codex` 调用，但上游返回 `429 Too Many Requests` 无可用产出，主线程完成实施并用 focused tests、类型检查、构建、artifact verifier、敏感路径扫描和 diff check 完成审核。

[2026-06-17 10:51:31 CST] [supaoauth] [DONE] feat(account-password-self-service): 补齐用户自助修改密码闭环。产品决策参考 Logto 形态：默认提供通用托管网页，同时暴露后端接口给业务应用自定义 UI；本次先落地最小安全闭环 `/account/password`、`/change-password`、`/change-password.html` 与 `POST /v1/public/account-password/change`，页面复用 sign-in experience branding，接口使用邮箱 + 当前密码先走 GoTrue password grant 校验，再用返回的用户 access token 调 GoTrue `/auth/v1/user` 更新密码，不使用 service-role/admin token。同步 SupaCloud Function route manifest、installer gateway routes、artifact/installed verifiers、embedded hosted pages 与回归测试。刻意未开放既有 `/v1/my-account` 给普通用户，因为该路由当前仍依赖 `x-supaoauth-user-id`，后续应单独做用户 JWT 鉴权版 Account API。验证：`bun test packages/auth-server/src/__tests__/account-password.test.ts packages/auth-server/src/__tests__/hosted-authorize-page.test.ts` 14 pass；`bun run --filter '@supauth/auth-server' typecheck` 通过；`bun run build` 通过并导出 OpenAPI 145 paths；`bun run scripts/verify-supacloud-app-artifact.ts` ok=true；`bun test tests/supacloud-app-contract.test.ts tests/supacloud-installed-app-verifier.test.ts` 13 pass；`bun run --filter '@supauth/auth-server' test` 205 pass；`bun run typecheck` 通过；`git diff --check` 通过。Delegation Gate: safe_skip_reason=小范围产品闭环和路由/静态页实现，涉及公开改密安全路径，主线程直接控密钥边界并用单元测试、hosted page 测试、类型检查、构建和 SupaCloud artifact/installed verifier 覆盖。

[2026-06-11 17:51:39 CST] [supaoauth] [DONE] fix(hosted-branding): 处理 `https://auth.ai.example.team/` 与 `/claim` 显示默认 SupAuth/SupaOAuth 品牌的问题，同时保持开源项目默认中性品牌。`/claim` 领取页现已读取 `/v1/public/sign-in-experience/resolve` 的 `branding.page_title` / `primary_color` / `logo_url` / `favicon_url` / `background_url` / `custom_css`；hosted 登录首页在无 `authorization_id` 时也会加载全局 sign-in experience branding，不再只在 OAuth 授权上下文中读取。线上 `supaoauth.sign_in_experience.page_title` 已从“示例 OAuth 用户中心”更新为“示例用户中心”，没有把示例字样写入开源默认 HTML 或 generated hosted pages。已构建并部署 vm1 `dglewlzugrtygzysqrce` SupAuth Function，备份 `/opt/supacloud/functions/dglewlzugrtygzysqrce/.manual-backups/supauth-branding-20260611174603`，`supacloud-edge-runtime.service` active。验证：`bun test packages/auth-server/src/__tests__/hosted-authorize-page.test.ts` 9 pass；`bunx tsc --noEmit` 通过；`bun run build` 通过；公网 `/v1/public/sign-in-experience/resolve` 返回 `page_title=示例用户中心`；Playwright 验证 `/claim` H1 与页面标题为“示例用户中心”，`/` H1 与页面标题为“示例用户中心”；installed verifier `ok=true`；`https://auth.ai.example.team/api/v1/health` 200；`git diff --check` 通过。Delegation Gate: verifier 子代理只读复核 PASS。后续 17:55 的账号批量导入部署仍保留该品牌脚本与线上配置。

[2026-06-11 17:55:30 CST] [supaoauth] [DONE] fix(account-claim-bulk-import): 修复用户反馈 `卿发英 / 0267` 在 `https://auth.ai.example.team/claim` 仍提示“未找到匹配账号”的问题，并补齐生产批量导入。根因有两层：生产租户库此前只有 `左华栋 / 0001` 1 条领取记录，员工清单中的 324 个正常员工尚未真正导入；同时导入清单保存的数字 ID 会被表格解析为 `267`，而用户按 4 位 `0267` 输入，旧查询只做精确匹配。代码侧 `normalizeExternalId()` 现在将纯数字 ID 收敛为去前导零 canonical 值，并在领取查询中兼容 canonical、原始输入和 4 位补零候选；`account:manifest` 生成时也把 1-3 位纯数字 ID 补成 4 位，避免后续清单继续漂移。生产侧已在 vm1 `dglewlzugrtygzysqrce` 使用 GoTrue ES256 service_role 管理 JWT 创建 323 个缺失 GoTrue 用户，写入 323 条未领取 `supaoauth.account_provisioning_records` 记录；已领取且姓名冲突的 `0001 / 左华栋` 被保护性跳过，未覆盖。发现首次导入密文使用了未解密 secret 后，立即用 Function secrets reveal 得到的真实运行时 secret 重新生成 323 组初始密码，同步更新 GoTrue 密码和 claim 加密字段，全程未输出密码/token/连接串。已构建并部署 SupAuth Function version `13`，备份 `/opt/supacloud/functions/dglewlzugrtygzysqrce/.manual-backups/supauth-claim-id-compat-20260611182000`，`supacloud-edge-runtime.service` active。验证：生产库 employee claim 记录 `total=324 / claimable=323 / claimed=1`；`卿发英` 记录为 `external_id=267 email=qingfaying@example.team initial_password_claimed=false has_initial_password=true`，GoTrue 用户存在且有密码；用当前 Function secret 可解密该记录密文；公网临时 smoke 记录以库存 `999`、页面输入 `0999` 返回 200 `claimed` 且有初始密码，随后已删除临时记录；`bun test packages/auth-server/src/__tests__/account-provisioning.test.ts` 8 pass；`bunx tsc --noEmit` 通过；`bun run --filter '@supauth/auth-server' build` 通过；`SUPAUTH_INSTALLED_BASE_URL=https://auth.ai.example.team SUPAUTH_INSTALLED_RUNTIME_URL=https://dglewlzugrtygzysqrce.ai.example.team bun run scripts/verify-supacloud-installed-app.ts` 通过且 installed verifier `ok=true`；`git diff --check` 通过。Delegation Gate: safe_skip_reason=线上领取数据修复与小范围 ID 规范化兼容，生产密钥/一次性密码路径要求主线程直接控输出，已用单元测试、类型检查、构建、生产 DB、密文解密、公网临时领取和 installed verifier 覆盖。

[2026-06-11 17:32:19 CST] [supaoauth] [DONE] fix(account-claim-live): 继续处理 `https://auth.ai.example.team/claim` 输入 `左华栋 / 0001` 仍提示“未找到匹配账号”的线上问题。复核生产租户库确认 `supaoauth.account_provisioning_records` 中 `employee / 0001 / 左华栋` 记录存在且 GoTrue 用户 `zuohuadong@example.team` 存在；用临时无密码记录验证公网 `/v1/public/account-claims/claim` 能读到同一张表。诊断期间为避免消耗真实一次性密码，曾短暂把该记录加密字段置空做匹配验证，随后使用 Edge Runtime 实际注入的 `SUPACLOUD_INTERNAL_TOKEN` 重新生成一组新的初始密码，同时写回 SupAuth claim 加密字段与 GoTrue bcrypt 密码，未输出密码。后续公网复核显示该账号已发生一次真实领取，当前接口对 `左华栋 / 0001` 返回 200 `already_claimed` 和邮箱，不再 404；如果用户没有看到一次性密码，需要管理员重置。另修复领取页前端错误映射：5xx 现在显示“系统暂时无法完成查询，请稍后重试或联系管理员。”，不再误报为姓名/数字 ID 不匹配；已构建并部署 vm1 `dglewlzugrtygzysqrce` SupAuth Function version `12`，备份 `/opt/supacloud/functions/dglewlzugrtygzysqrce/.manual-backups/supauth-claim-error-20260611173136`，`supacloud-edge-runtime.service` active。验证：公网 `/claim` 包含 `window.__SUPAOAUTH_PUBLIC_API_BASE__ = "/v1/public"` 与 `claimErrorMessage` 5xx 文案且不含 `http://auth.ai.example.team/v1/public`；公网 `POST /v1/public/account-claims/claim` 对 `左华栋 / 0001` 返回 200 `already_claimed`；临时加密领取记录返回 200 `claimed` 后已删除；`https://auth.ai.example.team/api/v1/health` 200；installed verifier `ok=true`；`bun test packages/auth-server/src/__tests__/hosted-authorize-page.test.ts` 9 pass；`bunx tsc --noEmit` 通过；`bun run build` 通过；`bun test` 268 pass / 42 skip；`git diff --check` 通过。当前生产 claim 数据总数仍为 1，`claimable=0`、`claimed=1`，除 `左华栋 / 0001` 外其他用户尚未导入领取记录。

[2026-06-11 17:15:21 CST] [supaoauth] [DONE] fix(account-claim-data): 处理用户反馈 `左华栋 / 0001` 在 `https://auth.ai.example.team/claim` 提示“未找到匹配账号”的问题。排查确认 `/v1/public/account-claims/claim` 对该输入返回 404，根因不是 mixed-content 或姓名规范化代码，而是生产租户库 `supa_dglewlzugrtygzysqrce.supaoauth.account_provisioning_records` 当前为 0 条，且 `auth.users` 中没有 `左华栋` / `zuohuadong` / `0001` 对应用户。已在 vm1 生产租户库补齐 GoTrue email 用户 `zuohuadong@example.team`、`auth.identities` email identity，以及 SupAuth claim 记录 `external_type=employee external_id=0001 display_name=左华栋 normalized_display_name=左华栋 source_status=active initial_password_claimed=false`；初始密码随机生成，只写入 GoTrue bcrypt 与 SupAuth claim 加密字段，未在日志或回复中输出，也未调用公开 claim 接口提前消费。验证：DB 查询按 `employee + 0001 + 左华栋` 命中且 `has_initial_password=true`；GoTrue 用户有密码、`raw_app_meta_data.supaoauth.external_id=0001`；email identity 存在；使用当前 Function secret 验证 claim 加密字段可解密；`https://auth.ai.example.team/api/v1/health` 200。用户现在可重新打开 `/claim` 输入 `左华栋` 和 `0001` 自行领取邮箱和一次性初始密码。

[2026-06-11 17:01:03 CST] [supaoauth] [DONE] fix(claim-mixed-content): 修复公网 `https://auth.ai.example.team/claim` 领取账号页在 HTTPS 页面中发起 `http://auth.ai.example.team/v1/public/account-claims/claim` 导致浏览器 Mixed Content 拦截的问题。根因是 hosted page 服务端渲染使用 `new URL('/v1/public', request.url)` 注入绝对 API base，而 SupaCloud Edge runtime 传入应用层的 request URL 可能是 `http://auth.ai.example.team/claim`；现在 hosted authorize/claim 页面统一注入同源相对 `"/v1/public"`，避免依赖代理传入协议且不改变 public API 路径或响应格式。新增回归测试覆盖 HTTPS `/claim` 不再注入 `http://auth.ai.example.team/v1/public`。已构建并部署到 vm1 `dglewlzugrtygzysqrce` SupAuth Function version `11`，备份 `/opt/supacloud/functions/dglewlzugrtygzysqrce/.manual-backups/supauth-mixed-content-20260611165918`；`supacloud-edge-runtime.service` 重启后 active。验证：`bun test packages/auth-server/src/__tests__/hosted-authorize-page.test.ts` 9 pass；`bunx tsc --noEmit` 通过；`bun run build` 通过；离线 artifact verifier `ok=true`；公网 `curl https://auth.ai.example.team/claim` 显示 `window.__SUPAOAUTH_PUBLIC_API_BASE__ = "/v1/public"` 且无 `http://auth.ai.example.team/v1/public`；`https://auth.ai.example.team/api/v1/health` 200；installed verifier `ok=true`；Playwright 浏览器提交无效样例时请求为 `https://auth.ai.example.team/v1/public/account-claims/claim`，没有 mixed-content，只有预期的无效样例 404；`bun test` 265 pass / 42 skip；`git diff --check` 通过。Delegation Gate: safe_skip_reason=低风险单路由渲染修复，影响面限于 hosted HTML API base 注入，已用单元测试、构建、live curl、installed verifier 和浏览器请求验证覆盖。

[2026-06-10 15:10:30 CST] [supaoauth] [DONE] fix(hosted-auth-approve): 修复 hosted 登录成功后 `POST /v1/public/oauth/authorizations/:authorizationId/approve` 卡住并返回 500 的问题。根因是 approve 服务端校验用户和提交 GoTrue consent 时使用 runtime 根路径拼接 `/user`、`/oauth/.../consent`，当前 hosted gateway 对这些错误路径返回 200 空体，导致 `response.json()` 抛异常；现在新增 GoTrue API URL helper，优先使用 `SUPAUTH_PUBLIC_URL` / `AUTH_PUBLIC_URL` 对应的 `/auth/v1/*`，再回退 internal/runtime `/auth/v1/*`，并对 200 空体、上游异常、consent 失败返回受控 JSON。hosted 页面也改为展示 approve 返回的错误消息，不再把授权错误吞成“网络错误”。已部署到 vm1 `dglewlzugrtygzysqrce` SupAuth Function version `10`，备份 `/opt/supacloud/functions/dglewlzugrtygzysqrce/.manual-backups/supauth-approve-fix-20260610150942`；公网复测 `POST https://auth.ai.example.team/v1/public/oauth/authorizations/m6bwtyms5axpgevcryzmcmuslhm772ep/approve` 使用无效 token 返回受控 `401 {"error":"invalid_bearer_token"}`，不再 500，`/api/v1/health` 200，登录页已包含新错误处理。验证：`bunx tsc --noEmit` 通过；focused hosted/sign-in tests 15 pass；`bun run build` 通过；`bun test` 242 pass / 42 skip；`SUPAUTH_INSTALLED_BASE_URL=https://auth.ai.example.team SUPAUTH_INSTALLED_RUNTIME_URL=https://dglewlzugrtygzysqrce.ai.example.team bun run scripts/verify-supacloud-installed-app.ts` 通过且 installed verifier `ok=true`；`git diff --check` 通过。

[2026-06-10 15:02:30 CST] [supaoauth] [DONE] fix(hosted-auth): 修复 hosted 登录页带 `authorization_id` 调用 public sign-in resolve 返回 500 并误显示“本次登录请求已过期”的问题。根因是 SupAuth Function 项目角色缺少 GoTrue `auth.oauth_authorizations` 最小权限，线上报 `permission denied for schema auth`；已通过 vm1 本地 SupaCloud Management API 对项目 `dglewlzugrtygzysqrce` 补 `GRANT USAGE ON SCHEMA auth` 与 `GRANT SELECT, UPDATE ON TABLE auth.oauth_authorizations`，公网 `https://auth.ai.example.team/v1/public/sign-in-experience/resolve?authorization_id=expired-smoke-test` 已从 500 恢复 200。代码侧 `PROJECT_ROLE_GRANTS_SQL` 固化该授权，public resolve 捕获 authorization lookup 异常并返回 `authorization_error`，hosted 登录页遇 resolve 失败时禁用登录并展示“暂时无法校验本次登录请求，请返回应用重新发起登录。”，避免继续误导为账号/过期问题；`packages/admin-console/static/authorize.html`、`packages/admin-console/build/authorize.html` 与 `packages/auth-server/src/generated/hosted-pages.ts` 已同步。验证：`bunx tsc --noEmit` 通过；`bun test packages/auth-server/src/__tests__/hosted-authorize-page.test.ts tests/supacloud-app-contract.test.ts` 16 pass；`bun test` 241 pass / 42 skip；`bun run build` 通过；`git diff --check` 通过；`SUPAUTH_INSTALLED_BASE_URL=https://auth.ai.example.team SUPAUTH_INSTALLED_RUNTIME_URL=https://dglewlzugrtygzysqrce.ai.example.team bun run scripts/verify-supacloud-installed-app.ts` 通过且 installed verifier `ok=true`。

[2026-06-10 15:01:30 CST] [supaoauth] [DONE] harden(open-public-url): 继续把 SSO custom-domain 修复收敛为开源通用语义。新增中性生产配置 `SUPAUTH_PUBLIC_URL`（兼容 `AUTH_PUBLIC_URL`、`SUPAUTH_INSTALLED_BASE_URL`、`SUPAUTH_BASE_URL`、`OAUTH_PUBLIC_BASE_URL`），`NODE_ENV=production` 时要求显式 public auth URL，非法 public URL 在 `validateConfig()` 阶段报错；新增 `TRUST_PROXY_HEADERS`，默认不信任 `X-Forwarded-*`，只有部署方明确开启且代理会清洗这些头时才用于推导 public origin。安装器现在同时注入 `SUPAUTH_PUBLIC_URL` 与兼容的 `SUPAUTH_INSTALLED_BASE_URL`，verifier、release gate、kong verifier、capacity baseline 均支持 `SUPAUTH_PUBLIC_URL` / `AUTH_PUBLIC_URL`。`.env.example`、auth-server env example、deployment/runbook/capacity 文档同步改为推荐 `SUPAUTH_PUBLIC_URL`。已部署到 vm1 `dglewlzugrtygzysqrce` SupAuth Function version `8`，备份 `/opt/supacloud/functions/dglewlzugrtygzysqrce/.manual-backups/supauth-open-public-url-20260610145949`，项目 secrets 已写入 `SUPAUTH_PUBLIC_URL=https://auth.ai.example.team` 与兼容 `SUPAUTH_INSTALLED_BASE_URL`；公网 `/oauth/sso/authorize` 仍返回 `302 Location: https://auth.ai.example.team/auth/v1/oauth/authorize?...`。验证：focused config/sso/verifier/installer tests 34 pass；`bunx tsc --noEmit` 通过；`bun test` 241 pass / 42 skip；`bun run build` 通过；`SUPAUTH_PUBLIC_URL=https://auth.ai.example.team SUPAUTH_INSTALLED_RUNTIME_URL=https://dglewlzugrtygzysqrce.ai.example.team SUPAUTH_SSO_AUTHORIZE_PROBE_URL=... bun run scripts/verify-supacloud-installed-app.ts` 通过且 `sso_authorize_redirect_origin` 302 ok；`git diff --check` 通过。

[2026-06-10 14:54:30 CST] [supaoauth] [DONE] fix(sso-custom-domain): 修复 SupAuth `/oauth/sso/authorize` 服务端下一跳优先级，避免从 hosted gateway 进入后仍按 `config.oauthRuntimeUrl` 跳回项目 runtime 域名。`packages/auth-server/src/config/index.ts` 新增 `publicBaseUrl`，读取 `SUPAUTH_INSTALLED_BASE_URL` / `SUPAUTH_BASE_URL` / `AUTH_PUBLIC_URL` / `OAUTH_PUBLIC_BASE_URL`；`packages/auth-server/src/routes/sso-authorize.ts` 生成 GoTrue authorize URL 时优先使用显式自定义域名，缺省才从 `x-forwarded-host` / request origin 推导；安装器 `scripts/install-supacloud-app.ts` 同步把 `SUPAUTH_INSTALLED_BASE_URL` 注入 Function env。新增回归测试锁定“自定义域名优先于 request host”和“SSO Location 不泄漏 project runtime”。已部署到 vm1 `dglewlzugrtygzysqrce` SupaCloud Function：备份 `/opt/supacloud/functions/dglewlzugrtygzysqrce/.manual-backups/supauth-custom-domain-20260610145056`，当前 `supauth.config.json` version `7`，项目 secret 已补 `SUPAUTH_INSTALLED_BASE_URL=https://auth.ai.example.team`，`supacloud-edge-runtime.service` 与 `supacloud.service` active。公网复测同类 URL 返回 `302 Location: https://auth.ai.example.team/auth/v1/oauth/authorize?...`，不再跳 `https://dglewlzugrtygzysqrce.ai.example.team/...`。验证：`bun test packages/auth-server/src/__tests__/sso-authorize.test.ts tests/supacloud-installed-app-verifier.test.ts` 15 pass；`bunx tsc --noEmit` 通过；`bun run build` 通过；`bun test` 234 pass / 42 skip；`SUPAUTH_INSTALLED_BASE_URL=https://auth.ai.example.team SUPAUTH_INSTALLED_RUNTIME_URL=https://dglewlzugrtygzysqrce.ai.example.team SUPAUTH_SSO_AUTHORIZE_PROBE_URL=... bun run scripts/verify-supacloud-installed-app.ts` 通过且 `sso_authorize_redirect_origin` 302 ok；`git diff --check` 通过。
