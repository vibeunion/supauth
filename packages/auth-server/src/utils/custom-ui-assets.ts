...
function validCleanupKey(objectKey: unknown): objectKey is string {
  ...
  const assetsId = match?.[1];
  const path = match?.[3];
  if (!assetsId || !path || !ASSETS_ID_PATTERN.test(assetsId)) return false;
  const normalized = normalizeCustomUiAssetPath(path);
...
function validAuditPendingEvent(
  ...
  return typeof candidate["event_id"] === 'string'
    ...
    && [...CUSTOM_UI_AUDIT_EVENT_TYPES].some(type => type === candidate["event_type"])
...
export function parseCustomUiManifest(manifestValue: unknown): CustomUiManifest | null {
  ...
  if (!candidate["files"].every((file: unknown): file is CustomUiManifestFile => validManifestFile(file, assetsId))) return null;
  const manifestFiles = candidate["files"];
  ...
  if (lifecycleState !== 'active' && lifecycleState !== 'cleanup_pending' && lifecycleState !== 'objects_deleted') return null;
  ...
  const manifestFields = {
    ...
    files: manifestFiles,
    cleanup_pending_object_keys: pending,
    lifecycle_state: lifecycleState,
  } satisfies Omit<CustomUiManifest, 'audit_pending_event'>;
  if (pendingAudit !== null && !validAuditPendingEvent(pendingAudit, manifestFields)) return null;
...
function validCleanupBatch(batch: unknown): batch is CustomUiCleanupBatch {
  ...
  if (candidate["state"] !== 'reserved' && candidate["state"] !== 'pending'
    && candidate["state"] !== 'upload_outcome_unknown' && candidate["state"] !== 'cleanup_claimed') return false;
  ...
  return candidate["object_keys"].every((objectKey: unknown) => validCleanupKey(objectKey)
...
export function parseCustomUiCleanupQueue(queueValue: unknown): CustomUiCleanupQueue | null {
  if (!isRecord(queueValue)) return null;
  const candidate = queueValue;
  ...
  const batches = candidate["batches"];
  ...
  return { ...candidate, schema_version: 1, batches };
}
