...
describe('RBAC Compatibility Inspector', () => {
  ...
  it('does not report a safe JWT role check for invalid discovery algorithm metadata', async () => {
    globalThis.fetch = strictFetch(mock(() => Promise.resolve(Response.json({
      ...
      userinfo_endpoint: 'http://runtime.test/auth/v1/userinfo',
      jwks_uri: 'http://runtime.test/auth/v1/.well-known/jwks.json',
      id_token_signing_alg_values_supported: ['RS256', null],
    }))));

    const results = await runRBACCompatibilityChecks();

    expect(results.find(result => result.check_id === 'rb-4-jwt-role-check')).toMatchObject({
      status: 'warn',
      message: 'Cannot check JWT role claim strategy without a reachable OIDC discovery endpoint.',
    });
    expect(results.some(result => result.check_id === 'rb-4-gotrue-jwt-role-safe')).toBe(false);
  });
