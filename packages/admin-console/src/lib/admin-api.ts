...
async function createAdminApiError(response: Response): Promise<AdminApiError> {
  ...
  const retryExhausted = response.headers.get('X-Svadmin-Auth-Retry') === 'exhausted';
  const code = retryExhausted
    ? 'auth_retry_exhausted'
    : [responseRecord?.code, responseRecord?.error_code, nestedErrorRecord?.code, nestedErrorRecord?.error_code]
        .find((candidate): candidate is string => typeof candidate === 'string');
  const message = [responseRecord?.message, nestedErrorRecord?.message, responseRecord?.error_description, responseBody]
    .find((candidate): candidate is string => typeof candidate === 'string')
    || response.statusText
...
export async function adminApiRequest(path: string, options: RequestInit = {}): Promise<unknown> {
