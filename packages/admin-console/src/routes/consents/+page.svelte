<script>
  ...
  async function handleRevoke(id) {
    if (!confirm(t('Revoke this consent?'))) return;
...
<div class="flex items-center justify-between mb-6">
  <h2 class="text-2xl font-bold text-surface-900">{t('Consents')}</h2>
...
<section class="bg-white rounded-xl border border-surface-200 p-6 mb-6">
  <label for="application-id" class="block text-sm font-medium text-surface-700 mb-1">{t('Application client_id')}</label>
  <input id="application-id" bind:value={applicationId} class="w-full px-3 py-2 border border-surface-300 rounded-lg text-sm font-mono" placeholder={t('GoTrue OAuth client_id')}>
</section>

{#if consents.length === 0}
  <div class="bg-surface-50 rounded-xl border border-surface-200 p-8 text-center">
    <p class="text-surface-500">{t('No consent records loaded')}</p>
  </div>
{:else}
  <div class="space-y-3">
      ...
      <div class="bg-white rounded-xl border border-surface-200 p-5 flex items-center justify-between gap-4">
        <div class="min-w-0">
          ...
          <p class="text-xs text-surface-500 mt-1">
            {t('scope:')} {consent.scopeId || consent.scope_id || 'all'} · {t('org:')} {consent.organizationId || consent.organization_id || 'none'}
          </p>
          <p class="text-xs text-surface-400 mt-1">{t('granted:')} {new Date(consent.grantedAt || consent.granted_at).toLocaleString()}</p>
        </div>
        <button onclick={() => handleRevoke(consent.id)} class="text-sm text-red-500 hover:text-red-700">{t('Revoke')}</button>
      </div>
    {/each}
  </div>
{/if}
