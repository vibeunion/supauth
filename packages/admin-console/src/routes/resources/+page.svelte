  ...
  async function handleCreate() {
    try {
      await createResource({
        ...
        scopes: newResource.scopes.split(',').map(s => s.trim()).filter(Boolean).map(s => ({ name: s })),
      });
      showCreate = false;
        ...
        {#if resource.scopes?.length}
          <div class="mt-3 flex flex-wrap gap-2">
            {#each resource.scopes as scope (scope.id)}
              <span class="px-3 py-1 bg-brand-50 text-brand-700 rounded-full text-sm font-medium">{scope.name}</span>
            {/each}
          </div>
        {/if}
      </div>
