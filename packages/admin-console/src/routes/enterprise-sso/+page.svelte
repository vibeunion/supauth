...
{#if showCreate}
  <section class="bg-white rounded-xl border border-surface-200 p-6 mb-6 space-y-4">
    <div>
      <label for="connector-id" class="block text-sm font-medium text-surface-700 mb-1">{t('Connector ID')}</label>
    ...
    <div>
      <label for="domains" class="block text-sm font-medium text-surface-700 mb-1">{t('Domains')}</label>
    ...
    <div class="grid grid-cols-2 gap-4">
      <div>
        <label for="protocol" class="block text-sm font-medium text-surface-700 mb-1">{t('Protocol')}</label>
    ...
    <div>
      <label for="org-map" class="block text-sm font-medium text-surface-700 mb-1">{t('Org Mapping JSON')}</label>
    ...
    <div>
      <label for="role-map" class="block text-sm font-medium text-surface-700 mb-1">{t('Role Mapping JSON')}</label>
  ...
  <div class="space-y-3">
    {#each configs as config (config.id)}
      <div class="bg-white rounded-xl border border-surface-200 p-5">
        ...
        <p class="text-xs text-surface-400 mt-1">{t('Protocol:')} {config.ssoProtocol || config.sso_protocol} · {t('JIT:')} {(config.jitProvisioning ?? config.jit_provisioning) ? t('enabled') : t('disabled')}</p>
      </div>
    {/each}
  </div>
{/if}
