...
<h2 class="text-2xl font-bold text-surface-900 mb-6">Overview</h2>
...
{:else}
  ...
  <div class="grid grid-cols-3 gap-4 mb-8">
    <div class="bg-white rounded-xl border border-surface-200 p-5">
      ...
      <p class="text-xs text-surface-400 mt-2">Signing: {status?.signing_alg || 'N/A'}</p>
    </div>

    <div class="bg-white rounded-xl border border-surface-200 p-5">
      <p class="text-sm text-surface-500 mb-1">Issuer</p>
      <p class="text-sm font-mono text-brand-700 break-all">{status?.issuer || 'N/A'}</p>
      <p class="text-xs text-surface-400 mt-2">Migration: {status?.migration_status || 'N/A'}</p>
    </div>

    <div class="bg-white rounded-xl border border-surface-200 p-5">
      <p class="text-sm text-surface-500 mb-1">Project</p>
  ...
  <div class="bg-white rounded-xl border border-surface-200 p-6 mb-8">
    <h3 class="text-lg font-semibold text-surface-800 mb-4">OIDC Endpoints</h3>
    <div class="space-y-2">
      {#if discovery}
        {#each [
  ...
  {#if compatReport?.checks?.length}
    <div class="bg-white rounded-xl border border-surface-200 p-6 mb-8">
      <h3 class="text-lg font-semibold text-surface-800 mb-4">Supabase Compatibility</h3>
      <div class="space-y-2">
      ...
      <p class="text-xs text-surface-400 mt-3">{compatReport.passed}/{compatReport.total} checks passed</p>
    </div>
  {/if}
  ...
  {#if discovery}
    <div class="bg-white rounded-xl border border-surface-200 p-6">
      <h3 class="text-lg font-semibold text-surface-800 mb-4">Supported Capabilities</h3>
