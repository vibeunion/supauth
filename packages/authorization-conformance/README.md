...
Pass reviewed installation and RLS SQL to `checkAuthorizationSql`. It checks the ordinary effective-grant projection contract, fixed application binding, helper hardening, authenticated policy clauses, and the uncorrelated allowed-scope shape. It rejects package-owned permission tables, legacy role adapters, caller-controlled application IDs, row-scoped helpers, row-column casts, tautological policies, and `array_agg`/`ANY` defaults.
