 ...
 * Env: SUPACLOUD_API_URL, OAUTH_RUNTIME_URL, DATABASE_URL in .env
 */
...
describe('Supabase runtime compatibility', () => {
  it('/auth/v1/.well-known/openid-configuration returns GoTrue discovery', async () => {
  ...
  it('/auth/v1/.well-known/jwks.json returns JWKS', async () => {
  ...
  it('Management API health returns SupaOAuth response', async () => {
  ...
  it('/rest/v1/* is not occupied by SupaOAuth', async () => {
  ...
  it('/storage/v1/bucket is not occupied by SupaOAuth', async () => {
